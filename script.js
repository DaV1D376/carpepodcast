...
const defaultGuests = [
    ...
    },
    {
        ...
        bio: "Lorenzo Rubinetti è un esperto allenatore di pallacanestro italiano con una lunghissima carriera alle spalle. Negli anni ha allenato e fatto da responsabile tecnico per tantissime realtà, sia giovanili (maschili e femminili). Tra le squadre che ha allenato ci...",
        imageUrl: "https://images.unsplash.com/photo-1544723795-3cj5a26b1f2o?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
        videoUrl: "#"
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderAdminList() {
        ...
        if (guests.length === 0) {
            adminGuestsList.innerHTML = '<p style="color: #888;">Nessun ospite presente.</p>';
            return;
        }

        guests.forEach((guest, index) => {
            const item = document.createElement('div');
            item.style.cssText = "display: flex; justify-content: space-between; align-items: center; padding: 15px; border: 1px solid #ddd; background: #fff; border-radius: 4px;";
            item.innerHTML = `
    ...
    if (addGuestForm) {
        addGuestForm.addEventListener('submit', (e) => {
            ...
            if (!imageUrl) {
                imageUrl = "https://images.unsplash.com/photo-1511367461989-f85a21fda167?q=80&w=1000&auto=format&fit=crop";
            }
    ...
    if (statsForm) {
        const adminStats = JSON.parse(localStorage.getItem('carpeStats')) || defaultStats;
